...
namespace Engine {
    ...
    VulkanMemAllocSubsystem::VulkanMemAllocSubsystem(
        VkInstance       instance,
    ...
    void VulkanMemAllocSubsystem::Shutdown()
    {
        ...
        ENGINE_LOG_DEBUG("VulkanMemAlloc", "Shutdown");
    }

    VulkanImageAllocation VulkanMemAllocSubsystem::AllocateImage(const VulkanImageSettings& settings) const
    {
        ENGINE_ASSERT_MSG(IsInitialized(), "VulkanMemAllocSubsystem: not initialized");

        const VmaAllocationCreateInfo allocInfo{
            .flags = settings.allocationFlags,
            .usage = settings.memoryUsage,
        };
